...
def _j_0(r: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    return jnp.array(jnp.sinc(r / jnp.pi))
...
def create_j_l(order: int, dtype: onp.dtype = onp.float32):
    ...
    def j_l_upward(r: jnp.ndarray) -> jnp.ndarray:
        ...
        return (jnp.asarray(orders), jnp.asarray(derivatives))
    ...
    def j_l_Cai(r: jnp.ndarray) -> jnp.ndarray:
        ...
        order_1 = _j_1(r)
        prefactor = jnp.where(
            jnp.fabs(order_1[0]) >= jnp.fabs(order_1[1]),
    ...
    # to calculate, it pays off to define a custon jvp rule for them.
    @j_l.defjvp
