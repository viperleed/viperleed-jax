...
ParameterSpaceSize = namedtuple('parameter_space_size', ['n_geo', 'n_vib', 'n_occ', 'n_v0r'])

class CalculatorWithParameterSpaces:
    @case(tags='cu_111')
    def case_default_parameter_space(self, cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_parameter_space):
        # expected size
        parameter_space_size = ParameterSpaceSize(n_v0r=1, n_vib=1, n_geo=3, n_occ=0,)
        return cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_parameter_space, parameter_space_size

    @case(tags='cu_111')
    def case_more_layers(self, cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_state_after_init):
        ...
        # expected size
        parameter_space_size = ParameterSpaceSize(n_v0r=1, n_vib=1, n_geo=12, n_occ=0,)
        return cu_111_dynamic_l_max_tensor_calculator, parameter_space, parameter_space_size

    @pytest.mark.xfail(reason="broken for too small parameter spaces")
    @case(tags='cu_111')
    def case_all_layers(self, cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_state_after_init):
        slab, _ = cu_111_dynamic_l_max_state_after_init
        parameter_space = ParameterSpace(slab)
        ## GEOMETRY
        # symmetry constrained xyz movements ± 0.15 A for layer 2
        ...
        parameter_space.v0r_param.set_bound(V0rParamBound(-2., +2.))
        # expected size
        parameter_space_size = ParameterSpaceSize(n_v0r=None, n_vib=None, n_geo=None, n_occ=None,) # TODO: fill
        return cu_111_dynamic_l_max_tensor_calculator, parameter_space


    @pytest.mark.xfail(reason="broken for too small parameter spaces")
    @case(tags='cu_111')
    def case_no_free_vib(self, cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_state_after_init):
        slab, _ = cu_111_dynamic_l_max_state_after_init
        parameter_space = ParameterSpace(slab)
        ## GEOMETRY
        parameter_space.geo_params.fix_layer(4, z_offset=0.)
...
@parametrize_with_cases("calculator, parameter_space, expected_size", cases=CalculatorWithParameterSpaces)
...
def test_expected_parameter_size(calculator, parameter_space, expected_size):
    expected = (expected_size.n_v0r, expected_size.n_vib, expected_size.n_geo, expected_size.n_occ)
