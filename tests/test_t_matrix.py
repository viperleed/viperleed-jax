...
JIT_T_MATRIX_FUNC = jax.jit(t_matrix.vib_dependent_tmatrix,
                            static_argnames='l_max')

@fixture(scope='session')
@pytest.mark.parametrize('n,vib_amp',
...
@parametrize_with_cases("l_max, phaseshifts, energies, vib_amp, t_matrices, abs",
                        cases=TMatrixInputs)
def test_single_calculation(l_max, phaseshifts,
                            energies, vib_amp, t_matrices, abs):
...
@parametrize_with_cases("l_max, phaseshifts, energies, vib_amp, t_matrices, abs",
                        cases=TMatrixInputs)
def test_single_calculation_jit(l_max, phaseshifts,
                            energies, vib_amp, t_matrices, abs):
...
@parametrize_with_cases("l_max, phaseshifts, energies, vib_amp, t_matrices, abs",
                        cases=TMatrixInputs)
def test_vmap_energy(l_max, phaseshifts,
                     energies, vib_amp, t_matrices, abs):
    site_el_ids = list(range(phaseshifts._phaseshifts.shape[0]))
    for s in site_el_ids:
        calculated = t_matrix.vmap_energy_vib_dependent_tmatrix(
            l_max,
            phaseshifts._phaseshifts[s, :],
            energies,
            vib_amp)
