...
_DATA_PATH = (
    Path(__file__).parent.parent / 'test_data' / 'Cu_111' / 'dynamic_l_max'
)
_LARGE_DATA_PATH = LARGE_FILE_PATH / 'Cu_111' / 'dynamic_l_max'
_REFERENCE_FILE_PATH = (
    _LARGE_DATA_PATH / 'Cu_111_dynamic_l_max_TensErLEED_reference.npz'
)
...
def cu_111_dynamic_l_max_state_after_init(cu_111_dynamic_l_max_info):
    state_after_init = run_viperleed_initialization(
        cu_111_dynamic_l_max_info.input_path
    )
...
@fixture(scope='session')
def cu_111_dynamic_l_max_raw_phaseshifts(
    cu_111_dynamic_l_max_info, cu_111_dynamic_l_max_state_after_init
):
...
@fixture(scope='session')
def cu_111_dynamic_l_max_phaseshifts(
...
def cu_111_dynamic_l_max_read_tensor_zip(cu_111_dynamic_l_max_info):
    return read_tensor_zip(
        cu_111_dynamic_l_max_info.tensor_path,
...
@fixture(scope='session')
def cu_111_dynamic_l_max_read_ref_data(
    cu_111_dynamic_l_max_read_tensor_zip, cu_111_dynamic_l_max_state_after_init
):
    slab, _ = cu_111_dynamic_l_max_state_after_init
    non_bulk_atoms = [at for at in slab.atlist if not at.is_bulk]
    sorted_tensors = [
        cu_111_dynamic_l_max_read_tensor_zip[f'T_{at.num}']
...
@fixture(scope='session')
def cu_111_dynamic_l_max_tensor_calculator_recalc_t_matrices(
    cu_111_dynamic_l_max_state_after_init,
    cu_111_dynamic_l_max_phaseshifts,
    cu_111_dynamic_l_max_read_ref_data,
):
...
@fixture(scope='session')
def cu_111_dynamic_l_max_calculator_with_parameter_space(
    cu_111_dynamic_l_max_tensor_calculator, cu_111_dynamic_l_max_parameter_space
):
...
@fixture(scope='session')
def cu_111_dynamic_l_max_calculator_with_parameter_space_recalc_t_matrices(
    cu_111_dynamic_l_max_tensor_calculator_recalc_t_matrices,
